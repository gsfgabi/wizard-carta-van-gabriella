        ...
        <div className="relative" ref={menuRef}>
          ...
          {open && (
            <div className="absolute right-0 mt-2 sm:mt-3 w-48 sm:w-56 bg-white rounded-2xl shadow-xl z-50 overflow-hidden animate-fade-in border border-[#ececec] transition-all duration-200 ease-in-out" style={{ minWidth: 200, boxShadow: '0 8px 32px 0 rgba(141,68,173,0.10)' }}>
              ...
              <a
                href="/historico"
                className="flex items-center gap-2 sm:gap-3 px-4 sm:px-6 py-2 sm:py-3 text-[#8D44AD] font-normal bg-white transition-all duration-200 ease-in-out text-sm sm:text-base outline-none group rounded-none focus:outline-[#8D44AD] focus:outline-2 focus:z-10 hover:bg-[#8D44AD] hover:text-white focus:bg-[#8D44AD] focus:text-white"
                onClick={() => setOpen(false)}
                tabIndex={0}
                style={{ borderRadius: 0 }}
              >
                <ClockIcon className="h-4 w-4 sm:h-5 sm:w-5 text-[#8D44AD] group-hover:text-white group-focus:text-white transition-colors" style={{ minWidth: 16 }} />
                <span>Histórico</span>
              </a>
              <div className="h-px bg-[#ececec] mx-3 sm:mx-4" />
              <button
