...
export const Wizard: React.FC = () => {
  ...
  const [stepBeforeValidation, setStepBeforeValidation] = useState<WizardStep | null>(null);
  ...
  const handleNext = (triggeredByStep: WizardStep) => {
    ...
    if (triggeredByStep === "form") {
      ...
      if (availableVanTypes.length === 1) {
        console.log('Exactly one available VAN type found, skipping VAN type selection.');
        setSelectedVanTypes([availableVanTypes[0].id.toString()]);
        handleGenerateLetterAndNext([availableVanTypes[0].id.toString()], triggeredByStep);
        return;
      } else {
        console.log('Zero or more than one available VAN types found, proceeding to VAN type selection.');
        const nextStepName = Object.keys(stepMap).find(key => stepMap[key as WizardStep] === nextStepIndex) as WizardStep;
  ...
  const handleBack = (triggeredByStep: WizardStep) => {
    const triggeredStepIndex = stepMap[triggeredByStep];
    const prevStepIndex = triggeredStepIndex - 1;
    if (prevStepIndex >= 0) {
      const prevStepName = Object.keys(stepMap).find(key => stepMap[key as WizardStep] === prevStepIndex) as WizardStep;
      
      switch (triggeredByStep) {
        case "validation":
          if (stepBeforeValidation) {
            setCurrentStep(stepBeforeValidation);
            setExpandedStep(stepBeforeValidation);
          } else {
            setCurrentStep(prevStepName); 
            setExpandedStep(prevStepName);
          }
          break;
        case "completion":
          setCurrentStep("validation");
          setExpandedStep("validation");
          break;
        default:
          setCurrentStep(prevStepName);
          setExpandedStep(prevStepName);
  ...
  const handleGenerateLetterAndNext = (vanTypesSelected: string[], triggeredByStep: WizardStep) => {
        ...
        if (stepName === "bank") {
          ...
          return (
            <div className="text-black">
              Banco selecionado: <span className="font-semibold text-gray-800">{bank ? bank.name : 'Nenhum'}</span>
            </div>
          );
          ...
          return (
            <div className="text-black">
              Produtos selecionados: <span className="font-semibold text-gray-800">{selectedProductNames || 'Nenhum'}</span>
            </div>
          );
        } 
        return <div className="text-gray-600 italic">Visualizar resumo (Não implementado)</div>;
     }
      ...
      case "van-type":
        return (
          <VanTypeSelection
            ...
            onNext={(vanTypesSelected) => handleGenerateLetterAndNext(vanTypesSelected, stepName)}
            onBack={() => handleBack(stepName)}
          />
        );
      case "validation":
  ...
  return (
    <div className="min-h-screen bg-[#8D44AD] flex flex-col items-center justify-start px-2">
      <Stepper 
        ...
        steps={steps} 
        onStepClick={(index) => {
           const stepName = Object.keys(stepMap).find(key => stepMap[key as WizardStep] === index) as WizardStep;
           if (isStepCompleted(stepName) || stepName === currentStep) {
      ...
      <div className="w-full mt-6">
        {orderedSteps.map((step, index) => {
           ...
           return (
             <Card key={stepName} className="mb-4 w-full max-w-full sm:max-w-2xl md:max-w-3xl">
                <div 
                  className={`px-2 sm:px-6 md:px-8 py-1 flex items-center justify-between cursor-pointer ${!isCompleted && !isCurrent ? 'opacity-50 cursor-not-allowed' : ''}`}
                  onClick={() => handleStepHeaderClick(stepName)}
                >
                  <div className="flex items-center">
                     <div className={`rounded-full border-2 w-6 h-6 flex items-center justify-center mr-4 text-xs font-bold 
                        ${isCompleted ? 'bg-white border-[#8D44AD] text-[#8D44AD]' : isCurrent ? 'bg-white border-[#8D44AD] text-[#8D44AD]' : 'bg-transparent border-white text-white'}
                     `}>
                        {isCompleted ? <CheckIcon className="h-4 w-4" strokeWidth={3} /> : index + 1}
                     </div>
                   <h3 className="text-lg font-semibold text-black mr-2">{step.name}</h3>

                   {isCompleted && !isCurrent && !isExpanded && (
                      <span className="text-gray-800 text-lg ml-1">
                         {stepName === "bank" && selectedBank && 
                           `${banks.find(b => b.id === selectedBank)?.name || 'Nenhum'}`}
                         {stepName === "products" && selectedProducts.length > 0 &&
                            `${products
                               .filter(p => selectedProducts.includes(p.id.toString()))
                               .map(p => p.name)
                               .join(', ') || 'Nenhum'}`}
                         {stepName === "van-type" && selectedVanTypes.length > 0 &&
                            `${vanTypes
